...
export async function getActivity(userId: string) {
  try {
    ...
    const userQuips = await Quip.find({ author: userId });

    const childQuipIds = userQuips.reduce((acc, userQuip) => {
      return acc.concat(userQuip.children);
    }, []);

    const replies = await Quip.find({
      _id: { $in: childQuipIds },
      author: { $ne: userId },
    }).populate({
      path: "author",
